...
// language operates on a register, which can be thought of as the current value. The register is not part of the stack. 
...
// Consider a MULT operation in a stack-and-register language. It removes the value from the stack, 
...
// and a register value of 7, the MULT operation mutates the stack to [3, 6] (the 4 is removed), 
...
// Your function may assume that all arguments are valid programs — i.e., they will not do anything like trying to 
...
function minilang(program) {
  ...
  program.split(' ').forEach(function(commandWord) {
    var command = commands[commandWord];

    // if no command in commands hash matched, assign value to register
    command ? command() : register = parseInt(commandWord, 10);
  });
}
