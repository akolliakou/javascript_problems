...
// based on whether or not an inventory item is available. As before, 
...
function getTransactions(id, transactions) {
  return transactions.filter(transaction => transaction.id === id);
}

function isItemAvailable(id, transactions) {
  let itemTransactions = getTransactions(id, transactions);

  let total = itemTransactions.reduce((sum, {movement, quantity}) => {
    if (movement === 'in') {
      return sum + quantity
    } else {
      return sum - quantity;
